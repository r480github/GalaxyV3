function RammerheadEncode(baseUrl, prefix) {
  ...
  function get(url, callback, shush = false) {
    ...
    request.onload = function () {
      if (request.status === 200) {
        ...
        if (!shush) console.log('unexpected server response to not match "200". Server says "' + request.responseText + '"');
      }
    };
  }
  var api = {
    ...
    sessionexists(id, callback) {
      if (prefix) {
        get(prefix + "/sessionexists?id=" + encodeURIComponent(id), function (res) {
    ...
    shuffleDict(id, callback) {
      ...
      if (prefix) {
        get(prefix + "/api/shuffleDict?id=" + encodeURIComponent(id), function (res) {
